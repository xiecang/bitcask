...
func TestDB_getMergeFiles(t *testing.T) {
	type fields struct {
		options            Options
		mu                 *sync.RWMutex
	...
	tests := []struct {
		name               string
		fields             fields
		wantMergeFiles     []*data.File
		wantNonMergeFileId uint32
		wantErr            bool
	}{
		// TODO: Add test cases.
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			db := &DB{
				...
				olderFiles:         tt.fields.olderFiles,
				index:              tt.fields.index,
				seqId:              tt.fields.seqId,
				isMerging:          tt.fields.isMerging,
				isInitial:          tt.fields.isInitial,
				isSeqIdFileNotExit: tt.fields.isSeqIdFileNotExit,
				fileLock:           tt.fields.fileLock,
				bytesWrite:         tt.fields.bytesWrite,
				reclaimableSize:    tt.fields.reclaimableSize,
			}
			gotMergeFiles, gotNonMergeFileId, err := db.getMergeFiles()
			if (err != nil) != tt.wantErr {
				t.Errorf("getMergeFiles() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(gotMergeFiles, tt.wantMergeFiles) {
				t.Errorf("getMergeFiles() gotMergeFiles = %v, want %v", gotMergeFiles, tt.wantMergeFiles)
			}
			if gotNonMergeFileId != tt.wantNonMergeFileId {
				t.Errorf("getMergeFiles() gotNonMergeFileId = %v, want %v", gotNonMergeFileId, tt.wantNonMergeFileId)
			}
		})
	}
}

func TestDB_getNonMergeFileId(t *testing.T) {
	type fields struct {
		options            Options
		mu                 *sync.RWMutex
		activeFile         *data.File
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := db.getNonMergeFileId(tt.args.dirPath)
			if (err != nil) != tt.wantErr {
				t.Errorf("getNonMergeFileId() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("getNonMergeFileId() got = %v, want %v", got, tt.want)
			}
		})
	}
}

func TestDB_loadIndexFromHintFile(t *testing.T) {
	type fields struct {
		options            Options
		mu                 *sync.RWMutex
		activeFile         *data.File
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := db.loadIndexFromHintFile(); (err != nil) != tt.wantErr {
				t.Errorf("loadIndexFromHintFile() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}

func TestDB_loadMergeFiles(t *testing.T) {
	type fields struct {
		options            Options
		mu                 *sync.RWMutex
		activeFile         *data.File
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := db.loadMergeFiles(); (err != nil) != tt.wantErr {
				t.Errorf("loadMergeFiles() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
